...
(define (list-unsafe-modules path)
  (for ([p (in-list (directory-list path #:build? #t))]
        ...
        #:when (racket-file? p)
        #:when (readable? p))
    (with-handlers ([exn:fail:contract:counterexample? ((handle-contract-counterexample) p)] ;(λ (x) (printf "~a~n ~a~n~n" x p))]
                    [exn:fail:contract:maybe? (λ (x) (printf "~a~n ~a~n~n" x p))];((handle-contract-maybe) p)]
                    [exn:fail:contract? ((handle-contract-fail) p)] ;(λ (x) (printf "~a~n ~a~n~n" x p))];
                    ...
                    [exn:fail? ((handle-fail) p)] ;covers error (λ (x) (printf "~a~n ~a~n~n" x p))]
                    [exn? raise])
...
(module* main #f
  ...
  (list-unsafe-modules "/home/clay/work/progs"))
