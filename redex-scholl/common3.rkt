...
(provide
 ...
 ;; Any Any -> Boolean
 ;; (=Î±/racket e_1 e_2) determines whether e_1 is Î±-equivalent to e_2
...
;; -----------------------------------------------------------------------------
;; (=Î± e_1 e_2) determines whether e_1 and e_2 are Î± equivalent

(module+ test
  (test-equal (term (=Î± (lambda (x) x) (lambda (y) y))) #true)
  (test-equal (term (=Î± (lambda (x) (x 1)) (lambda (y) (y 1)))) #true)
  (test-equal (term (=Î± (lambda (x) x) (lambda (y) z))) #false))

(define-metafunction Lambda
  =Î± : any any -> boolean
  [(=Î± any_1 any_2) ,(equal? (term (sd any_1)) (term (sd any_2)))])

;; a Racket definition for use in Racket positions 
(define (=Î±/racket x y) (term (=Î± ,x ,y)))
